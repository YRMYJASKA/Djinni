 ...
 *  ----------------------------------------------------------
 *  This file has been merged from multiple headers. Please dont edit it directly
...
#define CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wexit-time-destructors\"") _Pragma("clang diagnostic ignored \"-Wglobal-constructors\"")
...
struct StringMaker<std::vector<T, Allocator>> {
    static std::string convert(std::vector<T, Allocator> const& v) { return ::Catch::Detail::rangeToString(v.begin(), v.end()); }
};

template <typename T>
struct EnumStringMaker {
    static std::string convert(const T& t) { return ::Catch::Detail::stringify(static_cast<typename std::underlying_type<T>::type>(t)); }
};
...
namespace Catch {
    ...
    public:
        ...
        auto operator==(bool rhs) -> BinaryExpr<LhsT, bool> const { return BinaryExpr<LhsT, bool>(m_lhs == rhs, m_lhs, "==", rhs); }
    ...
    namespace Detail {
        ...
        public:
            ...
            Approx& epsilon(T const& newEpsilon)
            {
                ...
                CATCH_ENFORCE(epsilonAsDouble >= 0 && epsilonAsDouble <= 1.0, "Invalid Approx::epsilon: " << epsilonAsDouble << ", Approx::epsilon has to be between 0 and 1");
...
namespace Catch {
    ...
    TestCase makeTestCase(ITestInvoker* testCase, std::string const& className, std::string const& name, std::string const& description, SourceLineInfo const& lineInfo);
}
...
namespace Catch {
    ...
    void AssertionHandler::handle(ResultWas::OfType resultType) { handle(resultType, nullptr, false); }
    ...
    auto AssertionHandler::shouldDebugBreak() const -> bool { return m_shouldDebugBreak; }
    ...
    void AssertionHandler::useActiveException() { handle(ResultWas::ThrewException, Catch::translateActiveException()); }
    ...
    void handleExceptionMatchExpr(AssertionHandler& handler, std::string const& str, StringRef matcherString) { handleExceptionMatchExpr(handler, Matchers::Equals(str), matcherString); }
    ...
    namespace clara {
        namespace detail {
            ...
            struct BoundValueRefBase : BoundRefBase {
                ...
                auto setFlag(bool) -> ParserResult override { return ParserResult::logicError("Flags can only be set on boolean fields"); }
            };
...
namespace Catch {
    ...
    bool SourceLineInfo::operator<(SourceLineInfo const& other) const noexcept { return line < other.line || (line == other.line && (std::strcmp(file, other.file) < 0)); }
...
namespace Catch {
    ...
    static SignalDefs signalDefs[] = { { SIGINT, "SIGINT - Terminal interrupt signal" }, { SIGILL, "SIGILL - Illegal instruction signal" }, { SIGFPE, "SIGFPE - Floating point error signal" }, { SIGSEGV, "SIGSEGV - Segmentation violation signal" }, { SIGTERM, "SIGTERM - Termination request signal" }, { SIGABRT, "SIGABRT - Abort (abnormal termination) signal" } };
...
namespace Catch {
    ...
    namespace {
        ...
        public: // IRegistryHub
            ...
            IExceptionTranslatorRegistry& getExceptionTranslatorRegistry() override { return m_exceptionTranslatorRegistry; }
...
namespace Catch {
    ...
    void RunContext::assertionStarting(AssertionInfo const& info) { m_reporter->assertionStarting(info); }
    ...
    bool RunContext::lastAssertionPassed() { return m_totals.assertions.passed == (m_prevPassed + 1); }
    ...
    void RunContext::runCurrentTest(std::string& redirectedCout, std::string& redirectedCerr)
    {
        ...
        try {
            ...
            if (m_shouldReportUnexpected) {
                AssertionHandler(m_lastAssertionInfo.macroName, m_lastAssertionInfo.lineInfo, m_lastAssertionInfo.capturedExpression, m_lastAssertionInfo.resultDisposition).useActiveException();
            }
        }
...
namespace Catch {
    ...
    auto StringRef::operator==(StringRef const& other) const noexcept -> bool { return size() == other.size() && (std::strncmp(m_start, other.m_start, size()) == 0); }
...
namespace Catch {
    ...
    bool isReservedTag(std::string const& tag) { return parseSpecialTag(tag) == TestCaseInfo::None && tag.size() > 0 && !std::isalnum(tag[0]); }
    ...
    TestCase makeTestCase(ITestInvoker* _testCase, std::string const& _className, std::string const& _name, std::string const& _descOrTags, SourceLineInfo const& _lineInfo)
    {
        bool isHidden = false;
...
namespace Catch {
    ...
    bool matchTest(TestCase const& testCase, TestSpec const& testSpec, IConfig const& config) { return testSpec.matches(testCase) && (config.allowThrows() || !testCase.throws()); }
...
namespace Catch {
    namespace TestCaseTracking {
        ...
        bool TrackerBase::TrackerHasName::operator()(ITrackerPtr const& tracker) const { return tracker->nameAndLocation().name == m_nameAndLocation.name && tracker->nameAndLocation().location == m_nameAndLocation.location; }
...
namespace Catch {
    ...
    {
        try {
            getMutableRegistryHub().registerTest(makeTestCase(invoker, extractClassName(classOrMethod), nameAndTags.name, nameAndTags.tags, lineInfo));
        } catch (...) {
            // Do not throw when constructing global objects, instead register the exception to be processed later
            getMutableRegistryHub().registerStartupException();
        }
...
namespace Catch {
    ...
    bool TestSpec::TagPattern::matches(TestCaseInfo const& testCase) const { return std::find(begin(testCase.lcaseTags), end(testCase.lcaseTags), m_tag) != end(testCase.lcaseTags); }
...
namespace Catch {
    ...
    auto getCurrentNanosecondsSinceEpoch() -> uint64_t { return std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now().time_since_epoch()).count(); }
    ...
    auto Timer::getElapsedNanoseconds() const -> unsigned int { return static_cast<unsigned int>(getCurrentNanosecondsSinceEpoch() - m_nanoseconds); }
    auto Timer::getElapsedMicroseconds() const -> unsigned int { return static_cast<unsigned int>(getElapsedNanoseconds() / 1000); }
...
namespace Catch {
    ...
    std::string StringMaker<signed char>::convert(signed char c) { return ::Catch::Detail::stringify(static_cast<char>(c)); }
...
namespace Catch {
    ...
    struct CompactReporter : StreamingReporterBase<CompactReporter> {
        ...
        void noMatchingTestCases(std::string const& spec) override { stream << "No test cases matched '" << spec << '\'' << std::endl; }
    ...
    private:
        ...
        void printTotals(const Totals& totals) const
        {
            if (totals.testCases.total() == 0) {
                ...
                stream << "Passed " << bothOrAll(totals.testCases.passed) << pluralise(totals.testCases.passed, "test case") << " with " << pluralise(totals.assertions.passed, "assertion") << '.';
            }
        }
    };
...
namespace Catch {
    ...
    public:
        ...
        void testCaseStarting(TestCaseInfo const& testCaseInfo) override { m_okToFail = testCaseInfo.okToFail(); }
        bool assertionEnded(AssertionStats const& assertionStats) override
        {
...
namespace Catch {
    ...
    public: // StreamingReporterBase
        ...
        void testGroupEnded(TestGroupStats const& testGroupStats) override
        {
            ...
            m_xml.scopedElement("OverallResults").writeAttribute("successes", testGroupStats.totals.assertions.passed).writeAttribute("failures", testGroupStats.totals.assertions.failed).writeAttribute("expectedFailures", testGroupStats.totals.assertions.failedButOk);
            m_xml.endElement();
        }
        ...
        void testRunEnded(TestRunStats const& testRunStats) override
        {
            ...
            m_xml.scopedElement("OverallResults").writeAttribute("successes", testRunStats.totals.assertions.passed).writeAttribute("failures", testRunStats.totals.assertions.failed).writeAttribute("expectedFailures", testRunStats.totals.assertions.failedButOk);
            m_xml.endElement();
        }
